...
# === Конфигурация прогнозирования и визуализации ===

HORIZON_DAYS: int = 30          # Прогноз на 30 дней вперёд
TEST_SIZE: int = 60             # Размер тестового набора (60 дней для валидации)
DATA_DIR: Path = Path("data")
DATA_DIR.mkdir(exist_ok=True, parents=True)  # Директория для хранения CSV-данных

LOG_PATH: str = "logs.txt"      # Файл для логирования результатов анализа
SEED: int = 42                  # Фиксированный seed для воспроизводимости результатов

# Стили визуализации для разных моделей (цвета, линии, подписи)
MODEL_STYLES = {
    "ridge": {"color": "#d62728", "linestyle": "-", "label": "Ridge"},
    "random_forest": {"color": "#2ca02c", "linestyle": "--", "label": "Random Forest"},
    "arima": {"color": "#9467bd", "linestyle": "-.", "label": "ARIMA"},
    "lstm": {"color": "#ff7f0e", "linestyle": ":", "label": "LSTM"},
    "actual": {"color": "#1f77b4", "linestyle": "-", "label": "Факт", "linewidth": 2},
}

# === Загрузка временных рядов из CSV ===

def _is_iso_date(s: str) -> bool:
    """Проверяет, соответствует ли строка формату ISO даты (YYYY-MM-DD)."""
...
def _load_series_from_multiline_header_csv(path: Path) -> pd.Series:
    """
    Загружает временной ряд из CSV с нестандартными/многострочными заголовками.
    Автоматически определяет структуру файла и извлекает цену (Adj Close/Close/Price).
    """
    raw = pd.read_csv(path, header=None)
    if raw.empty:
        raise ValueError("CSV пустой.")

    # Поиск строки с датами (либо по заголовку "Date", либо по формату даты)
    ...
    if date_row_idx is None:
        for i in range(len(raw)):
            ...
            if isinstance(val, str) and _is_iso_date(val):
                ...
                break
        if date_row_idx is None:
            raise ValueError("Не удалось определить структуру CSV: нет заголовка 'Date' или ISO-дат.")

    # Определение колонок и очистка данных
    ...
    # Преобразование типов и индексация по дате
    ...
    raise ValueError("В CSV нет колонки 'Adj Close', 'Close' или 'Price'.")


def load_series_from_csv(path: Path) -> pd.Series:
    """
    Надёжная загрузка временного ряда из CSV.
    Сначала пробует стандартный парсер pandas, при ошибке использует кастомный.
    """
    try:
...
def resolve_user_csv(user_id: int, ticker: Optional[str]) -> Path:
    """
    Находит CSV-файл пользователя: сначала по конкретному тикеру,
    затем последний доступный файл для пользователя.
    """
    if ticker:
    ...
    if candidates:
        return candidates[-1]
    raise FileNotFoundError("Не найдены CSV-файлы для пользователя. Выполните /getdata и пришлите тикер и сумму.")

    # === Метрики оценки качества и генерация признаков ===

def rmse(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    """Вычисляет Root Mean Square Error (RMSE) - метрику точности прогноза."""
...
@dataclass
class ModelResult:
    """Хранит результаты обучения модели и метрики качества."""
    name: str                  # Название модели
    rmse: float                # RMSE на тестовом наборе
    mape: float                # MAPE (%) на тестовом наборе
    test_pred: pd.Series       # Прогнозы на тестовом наборе с индексами дат
    model: Any                 # Обученная модель
    meta: Dict[str, Any]       # Метаданные (гиперпараметры, настройки)


def train_ridge(series: pd.Series, test_size: int, lags: int = 30) -> ModelResult:
    """Обучает Ridge-регрессию с лаговыми признаками для прогнозирования временного ряда."""
...
def train_rf(series: pd.Series, test_size: int, lags: int = 30) -> ModelResult:
    """Обучает Random Forest с лаговыми признаками для прогнозирования временного ряда."""
...
def train_arima(series: pd.Series, test_size: int) -> ModelResult:
    """Обучает классическую ARIMA(1,1,1) модель для прогнозирования временного ряда."""
...
def train_lstm(series: pd.Series, test_size: int, window: int = 30, epochs: int = 30, batch_size: int = 32) -> Optional[ModelResult]:
    """Обучает LSTM-сеть для прогнозирования временного ряда (PyTorch).
    Возвращает None, если PyTorch недоступен."""
    try:
        import torch
    ...
    # Прогнозирование и обратное масштабирование
    model.eval()
    with torch.no_grad():
...
# === Генерация прогнозов и выбор лучшей модели ===

def forecast_future_for_model(res: ModelResult, series: pd.Series, horizon: int) -> pd.Series:
    """
    Генерирует прогноз на заданное количество дней вперёд для выбранной модели.
    """
    last_date: pd.Timestamp = pd.Timestamp(series.index.max())
    start_date: pd.Timestamp = last_date + BDay(1)  # Следующий рабочий день
    idx: pd.DatetimeIndex = pd.bdate_range(start=start_date, periods=horizon)  # Рабочие дни

    if res.name in ("ridge", "random_forest"):
        # Рекурсивный прогноз с использованием лаговых признаков
        lags: int = int(res.meta["lags"])
        hist = series.copy()
        preds: List[float] = []
        current_date = start_date
        for _ in range(horizon):
            feat_row = [hist.iloc[-i] for i in range(1, lags + 1)]  # Создание лагов
            X_next = np.array(feat_row, dtype=float).reshape(1, -1)
            y_next = float(res.model.predict(X_next)[0])
            preds.append(y_next)
            hist.loc[current_date] = y_next  # Добавление прогноза в историю для следующего шага
            current_date = current_date + BDay(1)
        return pd.Series(preds, index=idx, name=f"y_future_pred_{res.name}")

    if res.name == "arima":
        # Прогноз с использованием обученной ARIMA модели
        fc = res.model.forecast(steps=horizon)
        return pd.Series(np.asarray(fc), index=idx, name="y_future_pred_arima")

    if res.name == "lstm":
        # Прогноз с использованием LSTM сети (рекуррентный прогноз)
        import torch
        ...
        with torch.no_grad():
            for _ in range(horizon):
                y_hat = model(seq)  # Прогноз следующего значения
                ...
                seq = torch.cat([seq[:, 1:, :], next_val], dim=1)  # Сдвиг окна
...
def train_all_and_select_best(series: pd.Series, test_size: int = TEST_SIZE) -> Tuple[ModelResult, Dict[str, ModelResult]]:
    """
    Обучает все доступные модели и выбирает лучшую по метрике RMSE.
    """
    ...
    best = min(results.values(), key=lambda r: r.rmse)  # Выбор модели с минимальным RMSE
    return best, results
...
def plot_all_models(history: pd.Series, forecasts: Dict[str, pd.Series], best_name: str) -> bytes:
    """
    Создает график с историческими данными и прогнозами всех моделей.
    """
    fig, ax = plt.subplots(figsize=(12, 6))
    history.plot(ax=ax, color="#1f77b4", linewidth=1.8, label="История")

    # Отображение прогнозов каждой модели со своим стилем
    ...
    ax.legend()
    ax.axvline(history.index.max(), color="gray", linestyle="--", alpha=0.7)  # Вертикальная линия текущего дня
...
def build_summary(history: pd.Series, forecast: pd.Series, model_name: str) -> str:
    """Формирует текстовую сводку с анализом прогноза."""
...
# === Торговая стратегия и логирование ===

def find_local_extrema(s: pd.Series) -> Tuple[List[pd.Timestamp], List[pd.Timestamp]]:
    """Находит локальные минимумы и максимумы в прогнозе для определения точек входа/выхода."""
    mins: List[pd.Timestamp] = []
    maxs: List[pd.Timestamp] = []
    for i in range(1, len(s) - 1):
        if s.iloc[i] < s.iloc[i - 1] and s.iloc[i] < s.iloc[i + 1]:
            mins.append(s.index[i])  # Локальный минимум
        if s.iloc[i] > s.iloc[i - 1] and s.iloc[i] > s.iloc[i + 1]:
            maxs.append(s.index[i])  # Локальный максимум
    return mins, maxs


def simulate_strategy(history_last_price: float, forecast: pd.Series, amount: float) -> Tuple[float, str]:
    """Симулирует торговую стратегию: покупка на минимумах, продажа на максимумах."""
    if amount <= 0:
    ...
    cash: float = amount  # Начальный капитал
    shares: float = 0.0   # Количество акций
    trades: List[str] = []  # История сделок

    # Проходим по всем дням прогноза
    for dt, price in forecast.items():
        p = float(price)
        if shares == 0 and dt in min_set:  # Сигнал на покупку
            shares = cash / p
            cash = 0.0
            trades.append(f"Покупка {shares:.4f} по {p:.2f} на {dt.date()}")
        elif shares > 0 and dt in max_set:  # Сигнал на продажу
            cash = shares * p
            trades.append(f"Продажа {shares:.4f} по {p:.2f} на {dt.date()}")
            shares = 0.0

    # Закрытие позиции в последний день, если остались акции
    if shares > 0:
    ...
    profit = cash - amount  # Расчёт прибыли
...
def append_log(user_id: int, ticker: str, amount: float, best: ModelResult, profit: float) -> None:
    """Записывает результаты анализа в лог-файл для отслеживания производительности."""
...
# === Telegram-хэндлеры для анализа данных и прогнозирования ===

def parse_args(args: List[str]) -> Tuple[str, float]:
    """Парсит аргументы команды /analyze: тикер и сумму инвестиций."""
    ticker: str = "Unknown"
    amount: float = 0.0
    if len(args) >= 1:
        ticker = args[0]
...
async def analyze_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Основной обработчик команды /analyze: выполняет полный цикл анализа."""
    try:
        user_id: int = update.effective_user.id if update.effective_user else 0
        ticker, amount = parse_args(context.args or [])

        # Этап1: Загрузка данных пользователя
        ...
        # Этап 2: Обучение и выбор лучшей модели
        best, all_results = train_all_and_select_best(series, test_size=TEST_SIZE)

        # Этап 3: Прогнозирование для всех моделей
        ...
        # Этап 4: Симуляция торговой стратегии
...
def register_stage2_5_handlers(app: Any) -> None:
    """Регистрирует обработчики команд этапов 2-5 (анализ и прогнозирование)."""
    app.add_handler(CommandHandler("analyze", analyze_cmd))


# Опционально: обработчик загрузки CSV-файлов

async def handle_csv(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Обрабатывает загруженные пользователем CSV-файлы с историей акций."""
...
def register_optional_csv_handler(app: Any) -> None:
    """Регистрирует обработчик для загрузки CSV-файлов (опционально)."""
    # ВАЖНО: PTB 20.6
